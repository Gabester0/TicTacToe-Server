...
const findGame = async (socket, io)=>{
   if(noGames){    //Handle Creating First Game
      ...
      await redis.set(`1`, JSON.stringify(gameStatus)) //Set A Game status of false because only one player has joined
      socket.join(`1`)
      ...
      const latestGameStateJSON = await redis.get(`${latestGame}`)
      ...
      if(latestGameState.status === false){    //Add Player to existing Game
         ...
         await redis.set(`${latestGame}`, JSON.stringify(updatedLatestGameState))
         ...
         await redis.set('games', updatedGamesJSON)

         const newGameState = { game: newGame, status: false};
         await redis.set(`${newGame}`, JSON.stringify(newGameState)) //Set A Game status of false because only one player has joined
         socket.join(`${newGame}`)
